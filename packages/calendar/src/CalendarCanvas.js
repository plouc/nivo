...
import { isCursorInRect, getRelativeCursor, degreesToRadians, useDimensions, useTheme, withContainer, useValueFormatter } from '@nivo/core'
...
const CalendarCanvas = memo(({
    ...
    const monthLegends = useMonthLegends({ months, direction, monthLegendPosition, monthLegendOffset })
    ...
    }, [canvasEl, innerHeight, innerWidth, outerWidth, outerHeight, pixelRatio, margin, days,
        dayBorderColor, dayBorderWidth, colorScale, yearLegend, yearLegends, monthLegend, monthLegends,
        legends, theme, formatLegend])

    const handleMouseHover = useCallback(
        event => {
            const data = findDayUnderCursor(event, canvasEl.current, days, days[0].size, dayBorderWidth, margin)
    ...
    const handleClick = useCallback(
        event => {
            ...
            const data = findDayUnderCursor(event, canvasEl.current, days, days[0].size, daySpacing, margin)
