...
export const icicleMapping = {
    ...
    schema: Joi.object<IcicleApiProps>().keys({
        ...
        label: Joi.string(),
        labelBoxAnchor: Joi.any().valid(...BOX_ANCHORS),
        labelAlign: Joi.any().valid('auto', 'start', 'center', 'end'),
        labelBaseline: Joi.any().valid('auto', 'top', 'center', 'bottom'),
        labelPaddingX: Joi.number(),
        labelPaddingY: Joi.number(),
        labelRotation: Joi.number(),
        labelSkipWidth: Joi.number().min(0),
        labelSkipHeight: Joi.number().min(0),
