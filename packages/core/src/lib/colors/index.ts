...
export const getColorScale = (colors: any, dataScale: any) => {
    if (isString(colors)) {
        const scheme = (colorSchemes as any)[colors]
        if (scheme !== undefined) {
            const scale = scaleOrdinal(scheme) as any
            scale.type = 'ordinal'
        ...
        if (dataScale !== undefined && colors.indexOf('seq:') === 0) {
            const interpolator = (colorInterpolators as any)[colors.slice(4)]
            if (interpolator !== undefined) {
                const scale = scaleSequential(interpolator).domain(dataScale.domain()) as any
    ...
    if (isArray(colors)) {
        const scale = scaleOrdinal(colors) as any
        scale.type = 'ordinal'
