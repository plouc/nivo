...
import { defsPropTypes, noop, withTheme, withDimensions, withMotion, getAccessorFor, getLabelGenerator, bindDefs, Container, CartesianMarkers, SvgWrapper, getRelativeCursor, isCursorInRect, clip, ResponsiveWrapper } from '@bitbloom/nivo-core';
...
import { inheritedColorPropType, ordinalColorsPropType, colorPropertyAccessorPropType, getOrdinalColorScale, getInheritedColorGenerator } from '@bitbloom/nivo-colors';
import PropTypes from 'prop-types';
import compose from 'recompose/compose';
import { BasicTooltip } from '@bitbloom/nivo-tooltip';
import { useAnnotations, Annotation } from '@bitbloom/nivo-annotations';
import uniqBy$1 from 'lodash/uniqBy';
...
var getLegendDataForKeys = function getLegendDataForKeys(bars, layout, direction, groupMode, reverse) {
  var data = uniqBy(bars.map(function (bar) {
    return {
      id: bar.data.id,
      label: bar.data.id,
      color: bar.color,
...
var getLegendDataForIndexes = function getLegendDataForIndexes(bars) {
  return uniqBy(bars.map(function (bar) {
    return {
...
var BarCanvas = function (_Component) {
  ...
  _createClass(BarCanvas, [{
    ...
    value: function draw(props) {
      ...
      var legendDataForKeys = uniqBy$1(result.bars.map(function (bar) {
        return {
          id: bar.data.id,
          label: bar.data.id,
          color: bar.color,
      ...
      var legendDataForIndexes = uniqBy$1(result.bars.map(function (bar) {
        return {
          id: bar.data.indexValue,
          label: bar.data.indexValue,
          color: bar.color,
