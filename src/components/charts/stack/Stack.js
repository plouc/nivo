...
class Stack extends Component {
    ...
    renderD3(props, state) {
        ...
        const stack = d3.layout.stack().offset(offset).values(d => d.values)
        ...
        // —————————————————————————————————————————————————————————————————————————————————————————————————————————————
        let paths = wrapper
            .select('.nivo_stack_areas')
            .selectAll('.nivo_stack_area')
            .data(stacked, d => d.index)
        ...
        paths
            .on('click', d => {
                ...
                if (filteredData.length > 1) {
                    this.setState({
                        excludeLayers: excludeLayers.concat([d.index]),
                    })
                }
            })
        ...
        // Hidden layers
        // —————————————————————————————————————————————————————————————————————————————————————————————————————————————
        const hiddenControls = wrapper
            .selectAll('.nivo_stack_hidden_control')
